import { ETHER_ADDRESS, EVM_REVERT, tokens, ether } from "./helper";
...
contract("Exchange", ([deployer, feeAccount, user1, user2]) => {
  ...
  describe("fee count and fee percentage", async () => {
  ...
  describe("deposite tokens", async () => {
    let result;
    let amount;
    beforeEach(async () => {
      amount = tokens(10);
      await token.approve(exchange.address, amount, { from: user1 });
      result = await exchange.depositeToken(token.address, amount, {
        from: user1,
      });
      describe("success", () => {
        it("track the token deposite", async () => {
          let balance;
          balance = await token.balanceOf(exchange.address);
          // check the token in exchange
          balance = await exchange.tokens(token.address, user1);
          balance.toString().should.equal(amount.toString());
        });
        it("Emint the desposite event", async () => {
          const log = result.log[0];
          log.event.should.eq("Deposit");
          const event = log.args;
          event.token.toString().should.equal(token.address.toString());
  ...
  describe("depositing Ether", () => {
    ...
    it("emits a Deposit event", async () => {
      ...
      event.token.should.equal(ETHER_ADDRESS, "token address is cottect");
  ...
  describe("checking user balance", async () => {
    ...
    it("return user balanceOf", async () => {
  ...
  describe("making orders", async () => {
    ...
    it('emits on "Order" event', async () => {
      ...
      event.tokenGet.should.equal(token.address, "tokenGive is correct");
      event.amountGet
        .toString()
        .should.equal(tokens(1).toString(), "amountGet is correct");
  ...
  describe("order action", async () => {
    ...
    describe("filling orders", async () => {
      ...
      describe("success", async () => {
        ...
        it("execute the trade & charges fees", async () => {
          ...
          balance
            .toString()
            .should.equal(tokens(1).toString(), "user1 received is correct");
          balance = await exchange.balanceOf(ETHER_ADDRESS, user2);
          balance
            .toString()
            .should.equal(
              ether(1).toString(),
              "user2 received Ether is correct"
            );
          balance = await exchange.balanceOf(ETHER_ADDRESS, user1);
          balance.toString().should.equal("0", "user2 Ether deduced");
          balance = await exchange.balanceOf(token.address, user2);
          balance
            .toString()
            .should.equal(
              tokens(0.9).toString(),
              "user2 address deducted with fee applied"
            );
        ...
        it("emit trade event", async () => {
          ...
          const event = log.args;
          event.id.toString().should.equal("1", "id is correct");
          event.user.toString().should.equal(user1, "user is cotrrect");
          event.tokenGet
            .toString()
    ...
    describe("cancelling orders", async () => {
      ...
      it('emits on "cancle Order" event', async () => {
        const log = result.logs[0];
